...
parser.add_argument('--points_size', type=int, default=100000,
                    help='Size of points.')
parser.add_argument('--points_uniform_ratio', type=float, default=0.9,
...
def main(args):
    np.random.seed(int(time.time()) & ((1 << 20) - 1))
    ...
    if args.n_proc != 0:
        ...
        #         res.get()
        print('Run with mutiprocessing: n_proc = ', args.n_proc)
        with Progress() as progress:
            task = progress.add_task('sampleing', total=len(input_files))
            with Pool(args.n_proc) as p:
                # print('enter p', input_files)
                results = [p.apply_async(process_path, (input_file, args))
                         for input_file in input_files]
